...
public class MainActivity extends Activity {
    static final int BLACK=Color.BLACK, WHITE=Color.rgb(246,246,246), MUTED=Color.rgb(165,170,177), PANEL=Color.rgb(34,38,46), CYAN=Color.rgb(0,184,212), RED=Color.rgb(255,77,86), REDDARK=Color.rgb(85,24,29);
    ...
    SharedPreferences p; CountDownTimer timer; TextView clock,msg,hold; boolean running=false,breaking=false,waiting=false; long pending=0,duration=0,nextMsg=MSG_EVERY; Runnable endRun;
    ...
    void home(){stop();running=false;breaking=false;keep(false);LinearLayout r=root();r.setGravity(Gravity.CENTER_HORIZONTAL);r.setPadding(dp(26),dp(42),dp(26),dp(30));add(r,txt("FOCUS TIMER",30,WHITE,true),0,0,0,0);TextView sub=txt("Distraction free. A better you.",15,MUTED,false);sub.setGravity(Gravity.CENTER);add(r,sub,0,8,0,28);int[] mins={15,25,30,45,60};for(int m:mins){Button b=button(m+" Minutes",PANEL);b.setOnClickListener(v->airplanePrompt(m));LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,dp(58));lp.setMargins(0,dp(6),0,dp(6));r.addView(b,lp);}TextView s=txt("⚙  Settings",16,MUTED,false);s.setGravity(Gravity.CENTER);s.setPadding(0,dp(28),0,dp(8));s.setOnClickListener(v->settings());r.addView(s,new LinearLayout.LayoutParams(-1,-2));setContentView(r);immersive();}

    void airplanePrompt(int m){long d=m*60000L;new AlertDialog.Builder(this).setTitle("Before you start").setMessage("For the best focus, turn on Airplane Mode. This helps block distractions.").setPositiveButton("Open Airplane Mode Settings",(x,w)->{pending=d;waiting=true;openAirplane();}).setNegativeButton("Continue anyway",(x,w)->startFocus(d)).show();}

    void startFocus(long d){duration=d;nextMsg=MSG_EVERY;running=true;breaking=false;keep(p.getBoolean("awake",true));LinearLayout r=root();r.setGravity(Gravity.CENTER_HORIZONTAL);r.setPadding(dp(24),dp(50),dp(24),dp(32));TextView label=txt("Focus Time",16,WHITE,false);label.setGravity(Gravity.CENTER);r.addView(label,new LinearLayout.LayoutParams(-1,-2));clock=txt(time(d),72,WHITE,true);clock.setGravity(Gravity.CENTER);add(r,clock,0,28,0,44);msg=txt("",23,WHITE,true);msg.setGravity(Gravity.CENTER);msg.setPadding(dp(14),dp(18),dp(14),dp(18));msg.setBackgroundColor(Color.rgb(27,30,36));msg.setVisibility(View.INVISIBLE);r.addView(msg,new LinearLayout.LayoutParams(-1,-2));hold=txt("Hold 3 seconds to end",14,MUTED,false);hold.setGravity(Gravity.CENTER);hold.setPadding(dp(12),dp(24),dp(12),dp(24));holdToEnd();add(r,hold,0,38,0,0);setContentView(r);immersive();timer=new CountDownTimer(d,250){public void onTick(long left){clock.setText(time(left));long elapsed=duration-left;if(p.getBoolean("messages",true)&&elapsed>=nextMsg){showMessage();nextMsg+=MSG_EVERY;}}public void onFinish(){complete();}}.start();}
    ...
    void complete(){running=false;stop();keep(false);feedback();LinearLayout r=root();r.setGravity(Gravity.CENTER);r.setPadding(dp(28),dp(34),dp(28),dp(34));r.setBackgroundColor(Color.rgb(25,5,7));TextView cup=txt("★",54,Color.rgb(255,170,0),true);cup.setGravity(Gravity.CENTER);r.addView(cup,new LinearLayout.LayoutParams(-1,-2));TextView done=txt("Well done!",30,WHITE,true);done.setGravity(Gravity.CENTER);add(r,done,0,18,0,10);TextView sub=txt("You completed your focus session.",18,MUTED,false);sub.setGravity(Gravity.CENTER);add(r,sub,0,0,0,32);Button br=button("Start 5 Minute Break",RED);br.setOnClickListener(v->startBreak());r.addView(br,btnLp());Button ap=button("Turn Off Airplane Mode",REDDARK);ap.setOnClickListener(v->openAirplane());r.addView(ap,btnLp());Button hm=button("Back to Home",PANEL);hm.setOnClickListener(v->home());r.addView(hm,btnLp());setContentView(r);immersive();}

    void startBreak(){running=false;breaking=true;keep(p.getBoolean("awake",true));LinearLayout r=root();r.setGravity(Gravity.CENTER);r.setPadding(dp(28),dp(34),dp(28),dp(34));r.setBackgroundColor(Color.rgb(25,5,7));TextView lab=txt("Break Time",17,Color.rgb(255,125,130),false);lab.setGravity(Gravity.CENTER);r.addView(lab,new LinearLayout.LayoutParams(-1,-2));clock=txt("05:00",72,Color.rgb(255,112,120),true);clock.setGravity(Gravity.CENTER);add(r,clock,0,26,0,42);TextView t=txt("Take a break.\nRecharge. Then get back to it.",18,WHITE,false);t.setGravity(Gravity.CENTER);r.addView(t,new LinearLayout.LayoutParams(-1,-2));Button end=button("End Break",REDDARK);end.setOnClickListener(v->home());add(r,end,0,44,0,0);setContentView(r);immersive();timer=new CountDownTimer(BREAK,250){public void onTick(long left){clock.setText(time(left));}public void onFinish(){breaking=false;feedback();home();}}.start();}
    ...
    View toggle(String name,String key,boolean def){LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(16),dp(10),dp(12),dp(10));row.setBackgroundColor(Color.rgb(27,30,36));TextView t=txt(name,16,WHITE,false);row.addView(t,new LinearLayout.LayoutParams(0,dp(52),1));Switch sw=new Switch(this);sw.setChecked(p.getBoolean(key,def));sw.setOnCheckedChangeListener((b,on)->p.edit().putBoolean(key,on).apply());row.addView(sw);LinearLayout box=new LinearLayout(this);box.setPadding(0,dp(5),0,dp(5));box.addView(row,new LinearLayout.LayoutParams(-1,-2));return box;}
    ...
    Button button(String s,int bg){Button b=new Button(this);b.setText(s);b.setTextSize(17);b.setTextColor(WHITE);b.setAllCaps(false);b.setTypeface(Typeface.DEFAULT_BOLD);GradientDrawable g=new GradientDrawable();g.setColor(bg);g.setCornerRadius(dp(28));b.setBackground(g);return b;}
